                    ...
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h4 class="modal-title" id="modal_formModalLabel"><?= $title ?></h4>
                            </div>
...
<script>
    ...
    grid.init({
        ...
        dataTable: {// here you can define a typical datatable settings from http://datatables.net/usage/options 
            ...
            "ajax": {
                "url": "<?= base_url() ?>T_alokasi_anggaran/admin_ajax_list", // ajax source,
            },
            "aaSorting": [],
            "columnDefs": [{
                    "targets": -1,
            ...
            "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
				...
				var alokasi = Number(aData[5]).toLocaleString();
				$('td:eq(5)', nRow).text(alokasi);
                $('td:eq(0), td:eq(5)', nRow).attr("align", "right");
                $('td:eq(4), td:eq(6)', nRow).attr("align", "center");
            },
        }
    });
    ...
    $('#datatable tbody').on('click', '.detail', function (e) {
        ...
        $.ajax({
            cache: false,
            type: "get",
            url: "<?= base_url() ?>T_alokasi_anggaran/get_data_by_id_json",
            data: "id=" + id,
            success: function (response) {
				...
				var tahun_anggaran = data[0].tahun_anggaran;
				var jumlah = data[0].jumlah;
				
				$("#form #id_kendaraan").select2('val', id_kendaraan);
				$("form #tahun_anggaran").val(tahun_anggaran);
    ...
    $('#datatable tbody').on('click', '.delete', function (e) {
        ...
        if (confirm('Anda yakin menghapus data ini ?')) {
            $.ajax({
                cache: false,
                type: "get",
                url: "<?= base_url() ?>T_alokasi_anggaran/hapus_json",
                data: "id=" + id,
                success: function (response) {
                    var data = JSON.parse(response);
	...
	$(document).ready(function(e) {
        $("#jumlah").inputmask('IDR 999,999,999', {
			numericInput: true,
			rightAlignNumerics: false,
			greedy: false
		});
		$("#tahun_anggaran").inputmask({
            "mask": "9",
            "repeat": 4,
            "greedy": false
